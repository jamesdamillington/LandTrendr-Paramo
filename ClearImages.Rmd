...
#rcl0 <- c(0,0,1)  #in input tif 0 indicates no cloud-free images, so code 1 here as we want to count these 
#rcl1 <- c(1,Inf,0)  
#cloudrcl <- rbind(rcl0,rcl1)
...
JanApr_1010_sum <- aCC_summary(JanApr_1020)

plot(JanApr_1020)
hist(JanApr_1020)
print(JanApr_1010_sum)

```
So 92.7% of the study area has pixels with 3 or fewer missing years (and 97.8 with 4 or fewer). 
 
We likely need to discard the 2.2% of pixes with >4 missing years?
 
Let's quickly plot a map of location of pixels with >4 missing years
```{r}
 
#rcl0 <- c(0,4,0)  #in input tif 0 indicates no cloud-free images, so code 1 here as we want to count these 
#rcl1 <- c(4,Inf,1)  
#pixrcl <- rbind(rcl0,rcl1)
...
So now we see slightly higher counts/proportions for years with <=3 cloudy years (97.1% vs 92.7% using four-month window) and <=4 cloudy years (99.0% vs 97.8% using four-month window)
...
``` 
 ...
 We can see that the main issue is in the south of the study area, with a few other spots dotted to the north. 
...
Reclass 0, 1
Sum
Count cells with sum == 2, add to data

```{r}
#function to read, reclass, mask Raster
rrmRas <- function(yr, mons, pixrcl, studyras, dpath){
...
#TEST AND DEBUG HERE!!
seqCloudCount <- function(yrs, mons, pixrcl, studyras, seq_len, seqSummary, datapath){

  yrstack <- stack() 
  seq_summary <- seqSummary

  for(i in seq_along(yrs)){
    
    if(i+(seq_len-1) <= length(yrs))
    {
      ...
      for(j in i:(i+seq_len-1)){
        ...
        print(yrs[j])
        yrstack <- stack(yrstack, rrmRas(yrs[j], mons, pixrcl, studyras, datapath))
      }
      
      missing <- sum(yrstack) #sum stack
      counts = freq(missing, value=seq_len)  #value of seq_len mean pixel is 1 in all rasters
      
      seq_summary <- appendSeqData(cDt=seq_summary,start=yrs[i], end=yrs[j], len=seq_len, count=counts)
  
      if(yrs[i+(seq_len-1)] == tail(yrs,1)) break
    } else print("seq_len longer than remaining yrs")
  }
...
sS_l2_1014_JanDec <- seqSummary

sS_l2_1014_JanDec <- seqCloudCount(yrs, Months, cloudrcl0, buf_r, seq_len, sS_l2_1014_JanDec, path)

```
  
 
- Second, for cellIDs with count > 1 identify cells with 2 consecutive years missing (and which years)
  - do same for count >2 for 3 consecutive years missing
  - etc. for max count
- Third (possibly) for higher counts, identify cells with multiple 2+ consecutive years missing
