...
getFires <- function(myStack, myWin){
  
  #set 0 to NA in Year layer and create new raster
  all_Year <- calc(myStack[["Year"]], NAize)
  
  all_Year_in <- mask(all_Year, myStack[["Paramo"]])              #get only fires within paramo
  all_Year_out <- mask(all_Year, myStack[["Paramo"]], inverse=T)  #get only fires outside paramo
  
  fires_all <- lsm_c_np(all_Year)       #we still want to record this count as some fires will be split by the paramo boundary (i.e. in + out > all)
  fires_in <- lsm_c_np(all_Year_in)
  fires_out <- lsm_c_np(all_Year_out)
  
  fires_in <-  mutate(fires_in, Paramo="Inside") 
  fires_out <- mutate(fires_out,Paramo="Outside")
  fires_all <- mutate(fires_all,Paramo="All")
  
  fires_combined <- bind_rows(fires_in, fires_out, fires_all)
  
  fires_combined <- mutate(fires_combined, Window=myWin)  #add window for later plotting
...
analysis_level <- "fires"   #if analyse by fires (not by individual pixels) 
#analysis_level <- "pixels"  #if analyse by individual pixels
pairs <- F    #if pixels, should analysisbe paired between all year and third of a year, set TRUE 
...
for(scenario in scenario_list){
  ...
  if(analysis_level == "fires"){
    
    all_tib <- getFires(all_stack, "All")
    thr_tib <- getFires(thr_stack, "Third")
    
    AllDat <- bind_rows(all_tib, thr_tib)
    
    write_csv(AllDat, path=paste0(getwd(),"/Data/NBRanalysis/FireCounts/FireCounts_Pset",scenario,".csv"))
