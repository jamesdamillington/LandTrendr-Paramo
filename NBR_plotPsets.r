...
data_dir <- paste0(getwd(),"/Data/NBRanalysis/")  ## define the name of directory to save results
...
pdf(paste0(data_dir, "PreVal_Window.pdf"), width=10, height=10)
  
W %>%
  filter(Variable == "PreValue") %>%
  ...
  ylab("PreValue") +
  ggtitle("PreValue (median with 5th and 95th percentiles)") +
...
W %>%
  ...
  geom_pointrange(aes(ymin=q25, ymax=q75),
                  position = position_dodge(width = 0.75), size=0.33) +
  ylab("PreValue") +
  ggtitle("PreValue (mean with 25th and 75th percentiles)") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
pdf(paste0(data_dir, "PreVal_Paramo.pdf"), width=10, height=10)
