...
#raster_tmp_dir <- "/home/james/r_raster_tmp"  ## define the name of a temp directory where raster tmp files will be stored
...
ltr_res <- raster("Data/NBRanalysis/ltr_res.grd")
```
...
writeRaster(gfa_max, "Data/NBRanalysis/gfa_max.grd")
writeRaster(gfa_sum, "Data/NBRanalysis/gfa_sum.grd")
```

```{r}
gfa_max <- raster("Data/NBRanalysis/gfa_max.grd")
gfa_sum <- raster("Data/NBRanalysis/gfa_sum.grd")
...
for(scenario in scenario_list){
  ...
  for(grp in c("JanApr","AllYear")){
    ...
    ct <- ct[,2:6]  #drop 2011 (column 1) to leave 2012:2016
    ...
    write.csv(ct, paste0("Data/MODIS/Global_Fire_Atlas/PixelOverlap_",grp,"_",scenario,"-fa.csv"))
  }
}
...
for(scenario in scenario_list){
  ...
  for(grp in c("JanApr","AllYear")){
    ...
    m_tbl <- bind_rows(m_tbl, misses)
    
    hits <- po %>% 
      slice(2) %>%
      pivot_longer(cols=everything(), names_to="year",values_to="count") %>%
      mutate(pset=scenario,grp=grp)
    h_tbl <- bind_rows(h_tbl, hits)
    
    falarm <- po %>% 
      slice(3) %>%
      pivot_longer(cols=everything(), names_to="year",values_to="count") %>%
      mutate(pset=scenario,grp=grp)
    fa_tbl <- bind_rows(fa_tbl, falarm)
    
  }
}

write_csv(m_tbl, "Data/MODIS/Global_Fire_Atlas/PixelOverlap_Misses.csv")
write_csv(h_tbl, "Data/MODIS/Global_Fire_Atlas/PixelOverlap_Hits.csv")
write_csv(fa_tbl, "Data/MODIS/Global_Fire_Atlas/PixelOverlap_FalseAlarms.csv")

```

```{r}

plotCounts <- function(.dat){
...
Hall <- read_csv(paste0("Data/MODIS/Global_Fire_Atlas/PixelOverlap_Hits.csv"))
...
pdf("Data/MODIS/Global_Fire_Atlas/PixelOverlap_Hits.pdf", width=10, height=10)
plotCounts(Hall)
dev.off()

pdf("Data/MODIS/Global_Fire_Atlas/PixelOverlap_Misses.pdf", width=10, height=10)
plotCounts(Mall)
dev.off()

pdf("Data/MODIS/Global_Fire_Atlas/PixelOverlap_Fall.pdf", width=10, height=10)
plotCounts(Fall)
dev.off()
